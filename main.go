...
func main() {
	...
	if *performLogout {
		...
		go func() {
			...
			var interfaceValue = 0

			// How often to call heartbeat to prevent session from expiring
			heartbeat := time.NewTicker(3 * time.Minute)

		job:
			for {
				log.Println("Getting prompt items...")
				prompt, err := conducts.GetPromptItems(ctx, app, interfaceValue)
				if err != nil {
					handleFatal(err)
				}
				log.Printf("Got prompt items. (%d found)\n", len(prompt.Items))

				interfaceValue = prompt.Interface
			...
			sleep:
				for {
					...
					case <-heartbeat.C:
						...
						callHeartbeat(ctx, app, interfaceValue)
					}
				}
			}
		}()
...
func callHeartbeat(ctx context.Context, app *app.App, interfaceValue int) {
	...
	_, err := app.Client.Call(ctx, u, client.Webpage)
	if err != nil {
		handleError(err)
		return
	}
	log.Println("Called heartbeat.")
}
